					     ...
					     *parent, int offset)
{
	...
	if (parent == NULL || parent->element_type != t_structobject) {
		...
		targetaddr =
		    parent->arr.objects.base +
		    (offset * parent->arr.objects.stride) + cursor->addr.offset;
	...
	for (; *cp != '\0'; cp++) {
				...
				*pattr++ = '\0';
				...
				for (cursor = attrs; cursor->attribute != NULL;
					...
					json_debug_trace((2,
							  "Checking against %s\n",
							  cursor->attribute));
					if (strcmp(cursor->attribute, attrbuf)
					    == 0)
						break;
				}
		...
		case await_value:
			if (isspace((unsigned char)*cp) || *cp == ':')
				...
				substatus =
				    json_read_array(cp, &cursor->addr.array,
						    &cp);
				if (substatus != 0)
					return substatus;
				state = post_array;
 ...
 foundit:
			...
			if (lptr != NULL)
				...
				case t_array:
					break;
				case t_check:
					if (strcmp(cursor->dflt.check, valbuf)
					    != 0) {
						json_debug_trace((1,
